...
@router.post('')
def crear_proveedor(payload: ProveedorCreate, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA', 'EMPLEADO'))):
...
def actualizar_proveedor(id_proveedor: int, payload: ProveedorUpdate, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA', 'EMPLEADO'))):
...
def inactivar_proveedor(id_proveedor: int, payload: InactivarPayload, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA', 'EMPLEADO'))):
...
def actualizar_categorias_proveedor(
    ...
    user: Usuario = Depends(require_roles('DUEÑA', 'EMPLEADO')),
):
    return respuesta_ok(
