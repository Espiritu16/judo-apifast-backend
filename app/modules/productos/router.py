...
@router.post('')
def crear_producto(payload: ProductoCreate, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA'))):
...
def actualizar_producto(id_producto: int, payload: ProductoUpdate, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA'))):
...
def inactivar_producto(id_producto: int, payload: InactivarPayload, db: Session = Depends(get_db), user: Usuario = Depends(require_roles('DUEÑA'))):
