...
from app.modules.movimientos import repository
...
def registrar_movimiento(db: Session, payload: dict, user: Usuario) -> MovimientoInventario:
    ...
    parametro = db.get(ParametroInventario, payload['id_producto'])
    if not parametro:
    ...
    return movimiento
def listar_movimientos(db: Session) -> list[dict]:
    data = repository.list_all(db)
    return [
        {
...
def obtener_movimiento(db: Session, id_movimiento: int) -> dict | None:
    m = repository.get(db, id_movimiento)
    if not m:
        return None
    return {
        'id_movimiento': m.id_movimiento,
